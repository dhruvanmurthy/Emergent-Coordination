...
class PersonaAgent(Agent):
    ...
    def __init__(self, agent_id: int, model: str, temperature: float, persona_wrapper: PersonaWrapper, max_tokens: int):
...
class PersonaGameMaster(GameMaster):
    ...
    def __init__(self, mode: str = "mean", mystery_range: tuple = None, temperature: float = 0.7, 
                 ...
                 persona_wrapper: PersonaWrapper = None, persona_agent_max_tokens: int = 200):
        ...
        self.persona_wrapper = persona_wrapper
        self.persona_agent_max_tokens = persona_agent_max_tokens
    ...
    def add_agent(self, model: str) -> PersonaAgent:
        ...
        agent = PersonaAgent(
            ...
            model,
            self.temperature,
            self.persona_wrapper,
            max_tokens=self.persona_agent_max_tokens,
        )
...
async def run_single_config(agents, temp, run_id, batch_folder, model, client_type, mode, max_rounds, persona_file, persona_agent_max_tokens):
    ...
    try:
        ...
        game = PersonaGameMaster(mode=mode, temperature=temp, max_rounds=max_rounds, 
                               ...
                               persona_agent_max_tokens=persona_agent_max_tokens)
...
async def run_batch(batch_configs, batch_num, total_batches, batch_folder, model, client_type, mode, max_rounds, persona_file, persona_agent_max_tokens, max_concurrent=10):
    ...
    async def limited_run_config(config):
        async with semaphore:
            ...
            return await run_single_config(
                agents,
                temp,
                run_id,
                batch_folder,
...
async def main():
    ...
    if not isinstance(models, list) or not models:
        raise ValueError("persona_experiment.models must be a non-empty list")
    ...
    persona_file = get_required(section, "persona_file")
    persona_agent_max_tokens = get_optional(section, "persona_agent_max_tokens", 200)
    ...
    results_prefix = get_optional(section, "results_prefix", "persona+reasoning_experiment")
    experiment_type = get_optional(section, "experiment_type", "persona_experiment")

    print("🧩 Loaded settings from settings.py for persona_experiment")
