...
class Agent:
    ...
    async def make_guess(self, round_num: int, game_history: List[Round], guess_range: tuple, mode: str, total_agents: int) -> tuple[int, str, object, bool]:
        ...
        if round_num == 1:
            ...
            lane_hint = self._build_lane_hint(guess_range, total_agents)
            ...
            You are Agent {self.agent_id} of {total_agents}. {lane_hint}
            ...
            prompt = self._build_strategic_prompt(game_history, guess_range, mode, total_agents)
    ...
    def _build_strategic_prompt(self, game_history: List[Round], guess_range: tuple, mode: str, total_agents: int) -> str:
    ...
    def _build_lane_hint(self, guess_range: tuple, total_agents: int) -> str:
        """Create an agent-specific anchor to reduce first-round symmetry collapse."""
        low, high = guess_range
        span = max(1, high - low)
        lane_center = low + int(round(((self.agent_id + 0.5) / max(1, total_agents)) * span))
        lane_center = max(low, min(lane_center, high))
        return (
            f"Use an independent strategy; prefer starting near {lane_center} "
            "instead of defaulting to the midpoint unless feedback strongly supports it."
        )
    ...
    def _extract_number_robust(self, response, guess_range: tuple) -> int:
        ...
        raise ParsingError(f"Agent {self.agent_id}: Could not parse any number from model response")
...
class GameMaster:
    ...
    async def play_round(self, round_num: int) -> Round:
        ...
        tasks = [
            agent.make_guess(round_num, self.game_history, self.guess_range, self.mode, len(self.agents)) 
        ...
        for i, (guess, prompt, response, parsing_failed) in enumerate(results):
            ...
            # Save API response for every agent response (success or fallback).
            try:
